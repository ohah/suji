...
// ============================================
// Phase 17-B: CEF Views delegates
// ============================================
...
// ============================================
// Clipboard API — cef_clipboard.zig 로 분리(동작 무변경). main.zig __core__ 디스패치 +
// 각 SDK 호출부가 cef.clipboard* 를 그대로 쓰도록 re-export 한다.
// ============================================
...
const linux_shell = if (is_linux) struct {
    ...
    fn showItemInFolder(path: []const u8) bool {
        const uri = fileUriIfExists(path) orelse return false;
        defer g_free(@ptrCast(uri));
...
const linux_tray = if (is_linux) struct {
    ...
    fn emit(tray_id: u32, click: []const u8) void {
        if (tray_id == 0) return;
    ...
    fn create(title: []const u8, tooltip: []const u8, icon_path: []const u8) u32 {
        ...
        if (title.len > 0) _ = setTitle(id, title);
        if (tooltip.len > 0) {
            _ = setTooltip(id, tooltip);
        } else if (title.len > 0) {
            _ = setTooltip(id, title);
        }
        return id;
    }
...
pub fn setTrayTitle(tray_id: u32, title: []const u8) bool {
    ...
    if (comptime is_linux) return linux_tray.setTitle(tray_id, title);
    if (!comptime is_macos) return false;
...
pub fn setTrayTooltip(tray_id: u32, tooltip: []const u8) bool {
    ...
    if (comptime is_linux) return linux_tray.setTooltip(tray_id, tooltip);
    if (!comptime is_macos) return false;
    if (!g_trays_initialized) return false;
    const entry = g_trays.get(tray_id) orelse return false;
...
pub fn setTrayMenu(tray_id: u32, items: []const TrayMenuItem) bool {
    ...
    if (comptime is_linux) return linux_tray.setMenu(tray_id, items);
    if (!comptime is_macos) return false;
    if (!g_trays_initialized) return false;
...
pub fn destroyTray(tray_id: u32) bool {
    ...
    if (comptime is_linux) return linux_tray.destroy(tray_id);
    if (!comptime is_macos) return false;
    if (!g_trays_initialized) return false;
    const entry = g_trays.get(tray_id) orelse return false;
...
//
// Linux는 GTK3, Windows는 TaskDialog/commdlg/IFileOpenDialog 경로.
...
// ============================================
const win_file_dialog = if (builtin.os.tag == .windows) struct {
...
const win_dlg = if (builtin.os.tag == .windows) struct {
    ...
    fn showOpen(opts: OpenDialogOpts, response_buf: []u8) []const u8 {
        ...
        if (opts.can_choose_directories and !opts.can_choose_files) {
            return win_file_dialog.showFolder(opts, response_buf);
        }
    ...
    fn showSave(opts: SaveDialogOpts, response_buf: []u8) []const u8 {
        ...
        const esc_n = util.escapeJsonStrFull(path, &esc_buf) orelse 0;
        var w: std.Io.Writer = .fixed(response_buf);
        w.print("{{\"canceled\":false,\"filePath\":\"{s}\"}}", .{esc_buf[0..esc_n]}) catch return writeSaveCanceledResponse(response_buf, true);
        return w.buffered();
    }
} else struct {};
