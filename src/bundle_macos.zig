...
fn symlinkGpuLibs(allocator: std.mem.Allocator, app_name: []const u8) !void {
    ...
    for (libs) |lib| {
        const target = try std.fmt.allocPrint(allocator, "../Frameworks/Chromium Embedded Framework.framework/Libraries/{s}", .{lib});
        defer allocator.free(target);
        const link = try std.fmt.allocPrint(allocator, "{s}/Contents/MacOS/{s}", .{ app_name, lib });
        defer allocator.free(link);
        std.posix.symlink(target, link) catch |err| {
