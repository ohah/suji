...
bash tests/mobile-backends/run.sh       # 코어+Rust(staticlib)+Go(c-archive)+Zig
                                        ...
                                        # register_handler 왕복 62 케이스.
                                        # zig:http=std.http→localhost 평문,
...
52/52) + **`ios-e2e.sh`/`android-e2e.sh`**(실 디바이스 e2e: 실 UIPasteboard
...
렌더러-제어 경로에 파일을 쓰는 캡처/인쇄 cmd(`print_to_pdf` / `capture_page` /
`desktop_capturer_capture_thumbnail`)도 fs sandbox 우회를 막기 위해 동일
`allowedRoots` 경계로 게이트. 단 **opt-in**(이 API들은 그동안 무제한 출하 —
`allowedRoots` 미설정/`[]`이면 레거시 무제한 비파괴, 설정 시 `fs_write_file`
와 동일 경계 enforce → 설정한 fs 통제가 캡처 출력경로도 포함). backend 우회 동일.
