...
bash tests/e2e/run-python-packaged.sh   # packaged 임베드 CPython 가드 — suji build → 패키지 exe 실행 → 번들 libpython init(`[suji-python] started`) + frontend bind 검증 (dev 가 못 잡는 sentinel-gated <exe_dir>/python 번들 해석 경로; Windows CI 포함)
