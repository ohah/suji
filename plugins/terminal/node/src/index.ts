...
interface SujiBridge {
  ...
  /** Subscribe to a server-emitted event channel. cb receives (channel, dataJson). Returns a subId. */
  on(channel: string, cb: (channel: string, data: string) => void): number;
  /** Unsubscribe by subId returned from on(). */
...
function getBridge(): SujiBridge {
  ...
  if (!bridge) {
    throw new Error(
      "@suji/plugin-terminal-node: bridge not available. This module must run inside a Suji app (libnode embedding).",
    );
  }
  return bridge;
}
...
async function call(cmd: string, payload: Record<string, unknown>): Promise<any> {
  ...
  if (resp?.error) throw new Error(`terminal: ${resp.error}`);
  return resp?.result;
}
...
let dataSubId: number | null = null;
let exitSubId: number | null = null;

function ensureDataSub(): void {
  if (dataSubId !== null) return;
  dataSubId = getBridge().on("term:data", (channel, data) => {
    if (channel !== "term:data") return; // bridge fans out all events to all listeners
    let payload: { id?: number; data?: string };
    try {
      payload = JSON.parse(data);
    } catch {
      return;
    }
    if (typeof payload.id !== "number" || typeof payload.data !== "string") return;
...
function ensureExitSub(): void {
  if (exitSubId !== null) return;
  exitSubId = getBridge().on("term:exit", (channel, data) => {
    if (channel !== "term:exit") return;
...
export const terminal = {
  ...
  async start(opts: StartOptions = {}): Promise<number> {
    ...
    if (opts.cmd !== undefined) payload.shell = opts.cmd;
