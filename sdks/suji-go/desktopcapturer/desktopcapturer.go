...
func GetSources(types string) string {
	return suji.Invoke("__core__", buildGetSourcesRequest(types))
}
...
func CaptureThumbnail(sourceID, path string) string {
	return suji.Invoke("__core__", buildCaptureThumbnailRequest(sourceID, path))
}

func buildGetSourcesRequest(types string) string {
	return fmt.Sprintf(`{"cmd":"desktop_capturer_get_sources","types":"%s"}`, jsonesc.Full(types))
}

func buildCaptureThumbnailRequest(sourceID, path string) string {
	return fmt.Sprintf(
