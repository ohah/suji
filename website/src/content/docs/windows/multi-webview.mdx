...
description: 한 창에 여러 web 콘텐츠 합성 — Electron WebContentsView · Tauri v2 Webview 비교 + Suji 상태
---

한 OS 창 안에 여러 web 콘텐츠를 합성하는 패턴. 멀티탭 브라우저 / 사이드바 + 메인 (다른
origin) / OAuth 인-라인 / 위젯 임베드 같은 use case에 사용된다.

> **현 상태**: macOS/Linux/Windows 기본 경로는 CEF Views 기반 `windows.createView`다.
...
> attach되고, Linux/Windows child는 CEF overlay view로 붙는다. macOS/Linux/Windows 모두에서
> `createView`/bounds/visibility/destroy가 동작한다.
...
- **CEF Views 기반 구현**: CEF Views API(`CefWindow` + `CefBrowserView`)를 사용해
  multi-WebContentsView lifecycle을 구현했다. `destroyView` 후 child target 제거,
  host 생존, 남은 view 생존, recreate가 macOS/Linux/Windows에서 동작한다.
...
대부분의 데스크톱 앱은 multi-window로 충분하다. `windows.createView`는 한 창 안에서
