...
**참고**: CEF `set_zoom_level`은 navigation 시점에 적용 deferred — 즉시 호출한 `getZoomLevel`이 cache된 0 반환 가능.
...
`__window_url`은 Electron `event.sender.url`과 대응. 4개 SDK 모두
...
**Linux/Windows**: Linux는 CEF Views 경로에서 `frame:false`, drag/no-drag region, `transparent`, `parent`, `alwaysOnTop`, `resizable`, min/max, fullscreen, `backgroundColor`를 지원한다. Windows frameless parity는 향후 추가 예정이다.
