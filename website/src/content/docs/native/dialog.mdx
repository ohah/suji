...
Linux는 GTK3 `GtkMessageDialog` / `GtkFileChooserDialog`를 사용한다.
...
모든 함수는 Electron 두-인자 오버로드를 지원한다. 첫 인자로 `windowId: number`를 넘기면 macOS에서는 sheet, Windows/Linux에서는 free-floating 경로를 사용한다. 옵션 객체만 넘기면 free-floating.
...
Zig/Rust/Go/Node 4개 백엔드 SDK 모두 typed wrapper를 노출하며, frontend `@suji/api`와 동일한 응답 형식을 사용한다.
...
|---|---|---|---|
| ~~**Sheet modal**~~ | ~~free-floating only~~ | ✅ 구현됨 (windowId 첫 인자) | — |
...
시각적 동작 확인은 `examples/dialogs-demo`를 통한 수동 검증을 권장한다.
