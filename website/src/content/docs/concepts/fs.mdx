---
title: File System
description: 텍스트 파일 read/write + stat/mkdir/readdir — core API
---
...
**렌더러가 file path를 받는 native cmd**도 동일 `allowedRoots` 경계로 게이트된다 — 쓰기: `print_to_pdf` / `capture_page` /
...
`allowedRoots`를 설정한 앱에서는 `fs.*`와 동일한 traversal/boundary 검사를 적용해
