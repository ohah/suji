...
**렌더러-제어 경로 cmd**: path 를 받는 역사적-무제한 cmd 도 동일 `allowedRoots`
경계로 게이트된다 — 쓰기: `print_to_pdf` / `capture_page` /
`desktop_capturer_capture_thumbnail`, 읽기: `native_image_get_size` /
`native_image_to_png|jpeg`(임의 파일을 base64 로 렌더러에 반환 = 파일내용
유출). 단 **opt-in** — `allowedRoots` 미설정/`[]`이면 레거시 무제한(이 API들은
그동안 무제한 출하라 비파괴), 설정 시 `fs.*`와 동일 경계로 enforce(설정한 fs
통제가 이 경로 읽기/쓰기도 포함 → 신뢰불가 렌더러의 임의 파일 읽기/쓰기 차단).
backend 우회 동일.
