...
`#[suji::handle]` 매크로가 파라미터 타입이 `InvokeEvent`인 걸 감지하면 자동 파생해서 주입. 일반 필드는 기존처럼 이름으로 JSON 추출.
...
Frontend `@suji/api`의 `windows.*`와 같은 cmd JSON을 코어로 보낸다. 응답은 `Option<String>` JSON.
