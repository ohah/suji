...
| `PYPI_TOKEN` | PyPI `suji-stubs` 발행(`__token__` 비번) | sdk-publish.yml |
...
| `suji-stubs` (packages/suji-python) | PyPI | `twine upload` (PEP 561 타입 스텁) |

- 기본(`workflow_dispatch`, `dry_run=true`): `npm pack`/`npm publish
  --dry-run`/`cargo publish --dry-run`/go build·vet/`twine check` **검증만**,
  실제 발행 안 함.
- 실제 발행: `git tag sdk-vX.Y.Z && git push --tags` 또는
  `dry_run=false`. 단 레지스트리 토큰(`NPM_TOKEN`/`CARGO_REGISTRY_TOKEN`/
  `PYPI_TOKEN`) 미설정 시 해당 발행 step 은 경고 후 자동 skip(안전).
