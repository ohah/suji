...
| `npx @suji/cli` | -- | `create-tauri-app` | ✅ `packages/suji-cli`(스캐폴더 + `defineConfig`/JS·TS config loader + `suji` JS launcher) — `npx @suji/cli init <name> [--backend=none\|zig\|rust\|go\|node\|lua\|multi] [--frontend=react\|vue\|svelte\|solid\|preact\|vanilla\|next] [--toolchain=vite\|rsbuild\|next] [--pm=npm\|pnpm\|bun\|vp]`(`create-suji` 별칭, `vz`/`voidzero`/`viteplus`는 VoidZero Vite+ `vp` alias). 산출물 `init.zig` 동형(templates 사본, 단일출처=init.zig lockstep), 실제 평가되는 `suji.config.ts` source config + materialized `suji.json`, 12300 기본 포트, `.github/workflows/suji.yml`. Native는 `SUJI_CONFIG_LOADER` 또는 `node_modules/@suji/cli/bin/load-config.js`를 사용해 `suji.config.ts/mts/cts/js/mjs/cjs`를 JSON으로 평가하고, 로더 부재 시 JSON-compatible static fallback만 사용. 검증: `zig build test`, `zig build`, init CLI E2E 16개(native init, npx CLI, npm pack, JS launcher env, TS config loader, native TS config loader, Vite 6종, Rsbuild 2종, Next static export build). npm publish 는 토큰 대기 |
...
- [x] **Phase 3: 모바일 활성화**
  ...
  - [x] **모바일 http (`suji.http.fetch` 동등)** — 모바일 백엔드는 코어-독립이라
        ...
        aarch64-ios/-simulator/android-cross 컴파일·정적 링크 성공. **미검증
        (정직)**: 실기기·실 네트워크. `process.run` 은 iOS 샌드박스 fork/exec
        금지로 제외.
  - [x] **모바일 HTTPS/TLS** — Zig std `crypto.Certificate.Bundle.rescan` 에
        `.ios` 분기가 없어 iOS CA 번들이 비는 문제를 앱 번들 PEM 주입으로
        우회. Zig 백엔드가 `suji_zig_backend_set_ca_bundle_path(path)` 를
        노출하고 `https://` fetch 전에 해당 PEM 을 `std.http.Client.ca_bundle`
        에 로드한 뒤 `client.now` 를 채워 std 기본 재스캔을 건너뛴다.
        iOS `zig/build-lib.sh` 는 호스트 시스템 CA 를 `Vendor/cacert.pem` 으로
        스테이징하고 `Backends.swift` 가 `Bundle.main` 경로를 주입. Android 는
        Zig std Linux rescan 후보의 `/system/etc/security/cacerts` 로 유지.
        검증: 하니스가 누락 PEM 경로 오류 + 로컬 HTTPS fixture(자체 서명
        `localhost` cert를 CA PEM 으로 주입) GET 200/body 왕복.
