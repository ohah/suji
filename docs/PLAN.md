...
| 자동 업데이트 | autoUpdater | `updater` 플러그인 | 🟡 1차 — manifest 기반 update check + semver 비교 + native artifact download + SHA-256 검증 + macOS/Linux quit-and-install 교체 helper 구현(`auto_updater_check_update`/`auto_updater_verify_file`/`auto_updater_download_artifact`/`auto_updater_quit_and_install`). Frontend/Node/Zig SDK + 유닛 + system-integration E2E + 실제 quit-and-install E2E. 남은 것: 포맷별 unpack/install 정책(.dmg/.zip/.deb)과 Windows 적용 |
...
13. 🟡 **자동 업데이트** — manifest check + artifact download + SHA-256 verify + macOS/Linux quit-and-install 교체 helper 1차 완료. 남은 것: 포맷별 unpack/install 정책(.dmg/.zip/.deb)과 Windows 적용
