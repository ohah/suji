...
│  └── ...                                             │
│                                                      │
│  WindowApi v1 (C ABI raw dispatcher)                 │
│  ├── request_json("__core__ window cmd JSON")         │
│  └── free_response                                   │
└──────────────────────┬───────────────────────────────┘
                       │
        ┌──────────────┼──────────────┬────────────────┐
...
|--------|-----------|------------|-----|
| Zig | `suji.windows.*` → `WindowApi.request_json()` 우선 | O | X |
| Rust | `SujiCore.get_window_api()` raw dispatcher | O | X |
| Go | `SujiCore.get_window_api()` raw dispatcher | O | X |
| Node.js | `__core__`/bridge wrapper | O | X |
...
```zig
/// 기존 SujiCore 끝에 WindowApi getter를 tail-add.
...
`get_window_api`는 `BackendRegistry.global`과 `special_dispatch("__core__")`가 준비된 뒤에만
non-null을 반환한다. 준비 전에는 기존 SDK처럼 `invoke("__core__", ...)` 폴백을 사용한다.

## WindowApi v1 (C ABI raw dispatcher)

현재 구현된 C ABI 표면은 typed BrowserWindow 함수 포인터 묶음이 아니라, 기존
`__core__` window cmd JSON을 그대로 받는 raw dispatcher다.
...
응답 문자열은 코어가 소유하므로 호출자는 반드시 같은 table의 `free_response`로 해제한다.
Zig SDK의 `windows.*` wrapper는 이 table이 있으면 우선 사용하고, 없으면
`invoke("__core__", ...)`로 폴백한다.
...
- **문자열 반환**: v1 raw dispatcher 응답은 코어가 할당, 호출자가 `free_response`로 해제.
...
아래 typed vtable은 후속 ABI 확장 후보이며 현재 구현된 v1 표면이 아니다. 현재 SDK는
위 `WindowApi.request_json` dispatcher로 동일 동작을 제공한다.
...
| `contextBridge` | **미제공** — `window.__suji__`는 메인 월드에서 `Object.freeze` + 슬롯 봉인으로 하드닝 |
...
하는 API. Suji는 preload 자체를 제공하지 않고, 현재 CEF renderer 경로는
`window.__suji__`를 메인 월드에 바인딩한 뒤 **Object.freeze + non-writable /
non-configurable 슬롯 봉인**으로 변조를 막는다. Chrome isolated-world나 Electron
contextBridge와 동일한 별도 V8 context는 아직 아니다.

**이유**: Suji 렌더러는 Node API 노출이 없어 preload가 해결하려던 "Node API leak" 문제가
애초에 없음. contextBridge는 사용자가 필요한 API를 직접 골라 노출하는 책임이 있는데,
Suji는 렌더러에 Node API를 노출하지 않고 `window.__suji__` 하나만 제공하므로,
현재 단계에서는 frozen bridge 하드닝만으로 후속 isolated-world 구현 전까지의
표면을 최소화한다.
...
플러그인이 `BrowserWindow`/`WebContentsView`를 조작할 수 있도록 `SujiCore`에
getter를 제공한다. `get_io` 패턴과 동일하되, v1은 기존 `__core__` window cmd
JSON을 받는 raw dispatcher다.
...
pub const SujiCore = extern struct {
    // ... 기존 필드
    get_io: *const fn () callconv(.c) ?*const anyopaque,
    get_window_api: *const fn () callconv(.c) ?*const SujiWindowAPI,
};
```

Zig SDK는 이를 얇게 감싸고, 일반 `windows.*` wrapper는 table이 있으면 우선
사용하고 없으면 기존 `invoke("__core__", ...)`로 폴백한다.

```zig
if (suji.getWindowApi()) |api| {
    const resp = api.request_json("{\"cmd\":\"set_title\",\"windowId\":1,\"title\":\"Hello\"}");
    defer api.free_response(resp);
}
```

포인터 stability: `SujiWindowAPI`는 코어 소유 static table이다.
BackendRegistry 복사/이동 영향 없음. 후속으로 typed function을 추가할 때는
tail-add 방식으로 ABI 호환을 유지한다.
...
```zig
// 코어 (Zig) — id 기반 window cmd
_ = suji.windows.setTitle(id, "Hello");

// Rust SDK — OO wrapper (내부는 id + windows.* cmd)
...
```zig
_ = suji.windows.loadURL(id, "https://example.com");
...
- Plugin이 `suji.getWindowApi()`/`windows.*` 호출해서 WindowManager 동작 (dispatcher 연결 확인)
...
### 메모리 안전
- v1 `WindowApi.request_json` 응답은 코어가 `allocator.dupe()`로 복사본 할당
- 호출자는 반드시 같은 table의 `free_response`로 해제 (안 하면 누수)
- 후속 typed vtable의 `get_title`, `get_url` 등 문자열 반환 함수는 `free_string`
  같은 전용 해제 함수가 필요하다.
...
### SujiCore 하위 호환성
- `SujiCore` 중간에 필드를 추가하면 기존 백엔드 바이너리와 ABI 비호환
- **해결**: `get_window_api` getter는 **구조체 끝**에 tail-add. 기존 백엔드는 이 필드에 접근하지 않으므로 안전
- 새 SDK 버전부터 `get_window_api` 사용. 이전 SDK는 그냥 무시
- `backend_init`에서 전달받는 `SujiCore` 포인터의 크기가 달라도, 기존 필드 오프셋은 동일

### contextIsolation
- 현재 구현: `window.__suji__`는 메인 월드에 바인딩되지만 항상 `Object.freeze`되고,
  `window.__suji__` 슬롯은 `non-writable`/`non-configurable`로 봉인된다.
- 한계: Chrome isolated-world/Electron contextBridge와 동일한 별도 V8 context는 아직
  아니다. 진짜 isolated-world는 후속 CEF integration 조사 항목이다.
...
- `SujiCore.get_window_api` raw dispatcher table (구조체 끝에 배치)
...
### Phase 7: 보안/플랫폼 전용
- contextIsolation: 메인 월드 `Object.freeze`/슬롯 봉인 완료, 진짜 isolated-world는 후속
...
```
- WindowApi v1의 request_json/free_response 필드가 non-null
- request_json 호출 시 __core__ window cmd dispatcher로 디스패치
- 잘못된 ID 전달 → 모든 함수 no-op (크래시 없음)
- free_response(null) → no-op
```
