...
- **[medium]** `MenuItem constructor options completeness` — **Phase A(id, visible) 완료**(Menu PR-1) + **accelerator 완료**(Menu PR-2). 잔여 Phase B: role/icon(별도 PR — role=NSMenuItem selector 모델 변경/destructive 검증, icon=per-item fs-gate). Add support for high-value MenuItem fields in phases: (1) Phase A (trivial): `id` (string identifier, no UI side-effect), `visible` (boolean flag, reuse enabled logic pattern). (2) Phase B (moderate):
