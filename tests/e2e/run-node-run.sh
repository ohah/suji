#!/usr/bin/env bash
# `suji run main.js` embedded Node.js CLI E2E.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SUJI_LOG="${SUJI_LOG:-/tmp/suji-e2e-node-run.log}"
...
cd "$ROOT"
rm -f "$SUJI_LOG"
bun test tests/e2e/node-run.test.ts --timeout 30000 2>&1 | tee "$SUJI_LOG"
