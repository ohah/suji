...
test "state plugin: 빈 scope 문자열 → global 폴백" {
    var reg = loader.BackendRegistry.init(std.heap.page_allocator, std.testing.io);
    defer reg.deinit();
    reg.setGlobal();
    try loadStatePlugin(&reg);

    // scope="" → resolveScope이 "global"로 폴백.
    freeResp(&reg, invokePlugin(&reg, "{\"cmd\":\"state:set\",\"key\":\"k\",\"value\":\"v\",\"scope\":\"\"}"));

    // 명시적으로 scope 미지정한 get으로도 조회 가능 — 둘 다 global 경로.
    const g = invokePlugin(&reg, "{\"cmd\":\"state:get\",\"key\":\"k\"}");
    try std.testing.expect(std.mem.indexOf(u8, g.?, "\"v\"") != null);
    freeResp(&reg, g);
}

test "state plugin: JSON-unsafe 문자 포함된 scope → global 폴백" {
    var reg = loader.BackendRegistry.init(std.heap.page_allocator, std.testing.io);
    defer reg.deinit();
    reg.setGlobal();
    try loadStatePlugin(&reg);

    // scope에 \" 포함 — wire에서 escape돼서 들어왔어도 isScopeValidChars에서 거부.
    // request JSON 직접 조립: scope 값 "bad\"name" 을 JSON 문자열로 인코딩하면 "bad\"name"
    freeResp(&reg, invokePlugin(&reg, "{\"cmd\":\"state:set\",\"key\":\"unsafe1\",\"value\":\"v1\",\"scope\":\"bad\\\"name\"}"));
    freeResp(&reg, invokePlugin(&reg, "{\"cmd\":\"state:set\",\"key\":\"unsafe2\",\"value\":\"v2\",\"scope\":\"line1\\nline2\"}"));

    // 둘 다 global로 폴백돼서 저장됨.
    const g1 = invokePlugin(&reg, "{\"cmd\":\"state:get\",\"key\":\"unsafe1\"}");
    try std.testing.expect(std.mem.indexOf(u8, g1.?, "\"v1\"") != null);
    freeResp(&reg, g1);
    const g2 = invokePlugin(&reg, "{\"cmd\":\"state:get\",\"key\":\"unsafe2\"}");
    try std.testing.expect(std.mem.indexOf(u8, g2.?, "\"v2\"") != null);
    freeResp(&reg, g2);
}
