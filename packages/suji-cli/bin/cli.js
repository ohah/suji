...
const BACKENDS = new Set(["none", "zig", "rust", "go", "node", "lua", "multi"]);
const FRAMEWORKS = new Set(["react", "vue", "svelte", "solid", "preact", "vanilla", "next"]);
const TOOLCHAINS = new Set(["vite", "rsbuild", "next"]);
const PMS = new Set(["npm", "pnpm", "bun", "vp"]);
const COMPOSITE_TEMPLATES = new Set([
  "react_vite",
  "vue_vite",
  "svelte_vite",
  "solid_vite",
...
function toolchainsForFramework(framework) {
...
function resolveTemplate(frontend, toolchain) {
...
function templateFor(framework, toolchain) {
...
function sujiJson(name, backend, template, pm) {
  const base = {
...
function scaffoldBackend(projectName, backend, name) {
...
function scaffoldFrontend(projectName, template) {
...
function installFrontend(projectName, pm) {
...
const opts = await promptMissing(parseArgs(process.argv.slice(2)));
