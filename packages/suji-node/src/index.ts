...
export const clipboard = {
  ...
  async writeBookmark(title: string, url: string): Promise<boolean> {
    const r = await invoke<{ success: boolean }>('__core__', { cmd: 'clipboard_write_bookmark', title, url });
    return r.success === true;
  },
  ...
  async writeFindText(text: string): Promise<boolean> {
    const r = await invoke<{ success: boolean }>('__core__', { cmd: 'clipboard_write_find_text', text });
    return r.success === true;
  },
  /** Find 펜보드 텍스트 읽기 (Electron `clipboard.readFindText`). macOS only(Win/Linux 빈 문자열). */
  async readFindText(): Promise<string> {
    const r = await invoke<{ text: string }>('__core__', { cmd: 'clipboard_read_find_text' });
    return r.text ?? '';
  },

  /** 여러 포맷 한 번에 쓰기 (Electron `clipboard.write({text,html,rtf})`). macOS=atomic,
   *  Win/Linux=best-effort 단일(text 우선). */
...
export const powerMonitor = {
   ...
   *  → "nominal"|"fair"|"serious"|"critical". Win/Linux "unknown". */
  async getCurrentThermalState(): Promise<"nominal" | "fair" | "serious" | "critical" | "unknown"> {
    const r = await invoke<{ thermalState: "nominal" | "fair" | "serious" | "critical" | "unknown" }>('__core__', { cmd: 'power_monitor_thermal_state' });
...
export const nativeTheme = {
  ...
  async shouldUseInvertedColorScheme(): Promise<boolean> {
    const r = await invoke<{ invertedColorScheme: boolean }>('__core__', { cmd: 'native_theme_inverted_color_scheme' });
  ...
  async shouldDifferentiateWithoutColor(): Promise<boolean> {
    const r = await invoke<{ differentiateWithoutColor: boolean }>('__core__', { cmd: 'native_theme_differentiate_without_color' });
...
export const globalShortcut = {
   ...
   *  모두 성공 시 true, 하나라도 실패 시 false(성공분 유지, 롤백 없음).
   *  ※ Electron 은 void 반환 — suji 는 집계 bool 추가 제공. */
  async registerAll(accelerators: string[], click: string): Promise<boolean> {
  ...
  /** 모든 등록 단축키 일시 정지/재개 (Electron `globalShortcut.setSuspended`).
   *  등록 유지, trigger 발신만 차단(isRegistered 는 true 유지). */
  async setSuspended(suspended: boolean): Promise<boolean> {
    const r = await invoke<{ success: boolean }>('__core__', { cmd: 'global_shortcut_set_suspended', suspended });
    return r.success === true;
  },

  /** 현재 suspended 상태 (Electron `globalShortcut.isSuspended`). */
  async isSuspended(): Promise<boolean> {
    const r = await invoke<{ suspended: boolean }>('__core__', { cmd: 'global_shortcut_is_suspended' });
...
export const app = {
  ...
  async show(): Promise<boolean> {
    const r = await invoke<{ success: boolean }>('__core__', { cmd: 'app_show' });
    return r.success === true;
  },

  /** 앱이 frontmost(활성)인지 (Electron `app.isActive()`). macOS only(Win/Linux false). */
  async isActive(): Promise<boolean> {
    const r = await invoke<{ active: boolean }>('__core__', { cmd: 'app_is_active' });
    return r.active === true;
  },

  /** 앱이 hide 상태인지 (Electron `app.isHidden()`). macOS only(Win/Linux false). */
  async isHidden(): Promise<boolean> {
    const r = await invoke<{ hidden: boolean }>('__core__', { cmd: 'app_is_hidden' });
  ...
  async isEmojiPanelSupported(): Promise<boolean> {
    const r = await invoke<{ supported: boolean }>('__core__', { cmd: 'app_is_emoji_panel_supported' });
    return r.supported === true;
  },
  ...
  /** app:certificate-error 응답 — allow=true 허용 / false 거부. */
  async certificateErrorRespond(id: number, allow: boolean): Promise<boolean> {
    const r = await invoke<{ success: boolean }>('__core__', { cmd: 'certificate_error_respond', id, allow });
    return r.success === true;
  },
  ...
  async loginRespond(id: number, ok: boolean, username = '', password = ''): Promise<boolean> {
    const r = await invoke<{ success: boolean }>('__core__', { cmd: 'login_respond', id, ok, username, password });
    return r.success === true;
  },

  /** app:select-client-certificate 응답 — index(0-based) 선택, -1=기본. */
  async selectClientCertificateRespond(id: number, index: number): Promise<boolean> {
    const r = await invoke<{ success: boolean }>('__core__', { cmd: 'select_client_certificate_respond', id, index });
    return r.success === true;
  },
