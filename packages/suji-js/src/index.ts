...
export const globalShortcut = {
  ...
  async setSuspended(suspended: boolean): Promise<boolean> {
    const r = await coreCall<{ success: boolean }>({ cmd: "global_shortcut_set_suspended", suspended });
    return r.success === true;
  },

  /** 현재 suspended 상태 (Electron `globalShortcut.isSuspended`). */
  async isSuspended(): Promise<boolean> {
    const r = await coreCall<{ suspended: boolean }>({ cmd: "global_shortcut_is_suspended" });
...
export const nativeImage = {
  ...
  async isEmpty(path: string): Promise<boolean> {
    const r = await coreCall<{ isEmpty: boolean }>({ cmd: "native_image_is_empty", path });
  ...
  async isTemplateImage(path: string): Promise<boolean> {
    const r = await coreCall<{ isTemplate: boolean }>({ cmd: "native_image_is_template", path });
...
export const nativeTheme = {
  ...
  async shouldUseHighContrastColors(): Promise<boolean> {
    const r = await coreCall<{ highContrast: boolean }>({ cmd: "native_theme_high_contrast" });
...
export const session = {
  ...
  async setDownloadPath(path: string): Promise<boolean> {
    const r = await coreCall<{ success: boolean }>({ cmd: "session_set_download_path", path });
    return r.success === true;
  },
...
export const app = {
  ...
  async setAsDefaultProtocolClient(protocol: string): Promise<boolean> {
    const r = await coreCall<{ success: boolean }>({ cmd: "app_set_as_default_protocol_client", protocol });
    return r.success === true;
  },
  ...
  async isDefaultProtocolClient(protocol: string): Promise<boolean> {
    const r = await coreCall<{ success: boolean }>({ cmd: "app_is_default_protocol_client", protocol });
    return r.success === true;
  },
  ...
  async removeAsDefaultProtocolClient(protocol: string): Promise<boolean> {
    const r = await coreCall<{ success: boolean }>({ cmd: "app_remove_as_default_protocol_client", protocol });
    return r.success === true;
  },
  ...
  async getApplicationNameForProtocol(url: string): Promise<string> {
    const r = await coreCall<{ name: string }>({ cmd: "app_get_application_name_for_protocol", url });
    return r.name ?? "";
  },
  ...
  async getApplicationInfoForProtocol(url: string): Promise<{ name: string; path: string; icon: string }> {
    const r = await coreCall<{ name: string; path: string; icon: string }>({ cmd: "app_get_application_info_for_protocol", url });
