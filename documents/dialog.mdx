...
|---|---|---|---|
| `showMessageBox` | ✓ NSAlert | stub (response:0) | ✓ TaskDialogIndirect, MessageBoxW fallback |
| `showErrorBox` | ✓ NSAlert (critical) | no-op | ✓ MessageBoxW |
| `showOpenDialog` | ✓ NSOpenPanel | stub (canceled:true) | ✓ GetOpenFileNameW / IFileOpenDialog(folder) |
| `showSaveDialog` | ✓ NSSavePanel | stub (canceled:true) | ✓ GetSaveFileNameW |

Linux는 향후 GTK file chooser/message dialog 작업 전까지 graceful stub이다.
Windows는 Win32 native dialog를 사용한다. 단, parent window에 attach되는 sheet modal은
macOS 전용이며 Windows dialog는 free-floating modal로 뜬다.
...
호환성용이며 화면에 표시되지 않음. Windows는 `OPENFILENAMEW.lpstrFilter` 형식으로
필터 그룹을 전달한다. 확장자는 점 없이.
...
- **Linux는 stub**: showOpen/Save는 canceled 응답, messageBox는 default 응답, errorBox는 no-op.
- **Sheet modal은 macOS 전용**: Windows는 parent window attach 없이 free-floating dialog를 사용한다.
