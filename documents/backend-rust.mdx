...
Rust 백엔드는 런타임 자동 타입 추출 대신, 명시적으로 `#[derive(suji::Type)]`를 붙인
req/res 타입을 `SujiHandlers` module augmentation으로 내보낸다.
...
`.d.ts` 파일이다. `export_for("@suji/node")`를 쓰면 Node SDK module augmentation도
같은 방식으로 만들 수 있다.
...
모든 함수는 `invoke("__core__", ...)` 경유 raw JSON `Option<String>`을 반환한다.
