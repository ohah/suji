...
대상으로. 멀티 윈도우 동시 DevTools도 정확히 매핑 (각 DevTools → 자기 inspectee).
...
**find 결과 보고**: `cef_find_handler_t.OnFindResult`를 `window:find-result`
...
**capturePage**는 CEF 직접 미노출 → CDP `Page.captureScreenshot` + dev-tools observer 로 구현 완료(file-path 방식, `window:page-captured` 이벤트).

**Linux 주의**: PDF 인쇄용 `cef_print_handler_t.get_pdf_paper_size`(U.S. Letter) 등록 완료. macOS/Windows 는 네이티브 인쇄라 무영향. Linux CI 런타임 E2E에서 실제 PDF 파일 생성과 `%PDF-` 시그니처를 검증한다.
...
**Linux/Windows**: Linux는 CEF Views 경로에서 `frame:false`와 drag/no-drag region을 런타임 E2E로 검증한다. `transparent`/`parent` 등 macOS NSWindow 기반 옵션과 Windows frameless parity는 후속이다.
...
- 검증 스크립트: `tests/e2e/run-frameless-drag-region.sh` (macOS/Linux CEF Views).
