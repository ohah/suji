...
`window:resized`/`window:moved`, ready/title, show/hide, minimize/maximize 계열을
GitHub Actions runtime E2E로 검증한다. Windows는 아직 후속이다.
...
- **focus/blur는 app activation에 의존**. 다른 앱이 활성 중이면 key/active window 변경이
  일어나지 않을 수 있다. E2E는 비결정적이라 단위/정적 경로로만 보존한다.
- Linux E2E: `tests/e2e/run-window-lifecycle-events-cef-views.sh`가 Actions Linux runner에서
  CEF Views `setBounds` resize, lifecycle control, ready/title, show/hide를 검증한다.
  CEF Views의 min/max native getter는 headless runner에서 command 직후 lag가 있어,
  Suji가 CEF Views delegate에 command-side 상태를 보존해 `is_minimized`/`is_maximized`와
  restore/unmaximize 이벤트를 일관되게 라우팅한다.
- Windows는 후속이다. Win32 lifecycle message hook과 현재 Windows build 안정화가 필요하다.
