...
**캡처/인쇄 경로**: `print_to_pdf` / `capture_page` / `desktop_capturer_capture_thumbnail`
는 렌더러-제어 path에 파일을 쓰므로 동일 `allowedRoots` 경계로 게이트된다. 단
**opt-in** — `allowedRoots` 미설정/`[]`이면 레거시 무제한(이 API들은 그동안 무제한
출하라 비파괴), 설정 시 `fs.*`와 동일 경계로 enforce(설정한 fs 통제가 캡처
출력경로도 포함 → 신뢰불가 렌더러의 임의 파일쓰기 차단). backend 우회 동일.
