// zig 단독 변형 — Zig(.a 정적) 백엔드만 등록 (suji_zig_backend_*).
...
static const char *zig_backend_h(const char *ch, const char *j) {
...
static void zig_backend_f(const char *p) { suji_zig_backend_free((char *)p); }
...
Java_dev_suji_examples_android_SujiCore_nativeRegisterStaticBackends(
    ...
    suji_reg_backend("zig:ping", zig_backend_h, zig_backend_f);
    suji_reg_backend("zig:rev", zig_backend_h, zig_backend_f);
}
